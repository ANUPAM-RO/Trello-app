    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {boards.map((board) => (
                    <Link key={board.id} href={`/board/${board.id}`} className="group relative aspect-video bg-no-repeat bg-center bg-cover bg-sky-700 rounded-sm h-full w-full p-2 overflow-hidden" style={{ backgroundImage: `url(${board.imageThumbUrl})` }}>
...
BoardList.Skeleton = function SkeletonBoardList() {
    return (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            <Skeleton className="aspect-vedio h-full w-full p-2"/>
            <Skeleton className="aspect-vedio h-full w-full p-2"/>
            <Skeleton className="aspect-vedio h-full w-full p-2"/>
            <Skeleton className="aspect-vedio h-full w-full p-2"/>
            <Skeleton className="aspect-vedio h-full w-full p-2"/>
